...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'avocado-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('avocado').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('avocado')[service]);
